    ...
    df = pd.DataFrame(
        {
            ...
            "Product": np.random.choice(
                ["Credit card", "BNPL", "Personal loan"], size=100
            ),
            "Issue": np.random.choice(
                ["Billing dispute", "Delayed response"], size=100
            ),
...
def test_indexer_initialisation(dummy_chunked_csv):
    ...
    indexer = LangchainIndexer(
        df_chunks_path=dummy_chunked_csv, vector_store_dir=temp_dir
    )
