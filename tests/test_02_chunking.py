...
from scripts._02_chunking import Chunking
...
def dummy_data():
    ...
    # Cleaned narratives
    cleaned_narratives = [
        n.lower().strip().replace(".", "").replace(",", "") for n in narratives
    ]
    ...
    df = pd.DataFrame(
        {
            "Clean Narrative": cleaned_narratives,
            ...
            "Consumer Consent Provided?": np.random.choice(
                ["Consent Provided", "No", "Unknown"], size=200
            ),
...
def test_save_df(dummy_data, tmp_path):
    ...
    output_file = os.path.join(processed_dir, "test_output.csv")
